    ...
    <main className="mx-auto w-full max-w-6xl px-6 py-7 lg:px-8">
      ...
      {jobs === null ? <LoadingState label={t.loading} /> : jobs.length === 0 ? (
        ...
        <>
          ...
          {filteredJobs.length === 0 ? <section className="border border-dashed border-stone-300 bg-white p-8 text-center text-sm text-stone-600">{t.noMatchingHistory}</section> : <section className="overflow-hidden border border-stone-300 bg-white">
            ...
            <div className="divide-y divide-stone-200">
              {filteredJobs.map((job) => <article key={job.id} className="grid grid-cols-[minmax(0,1fr)_80px] gap-4 px-4 py-4 sm:grid-cols-[minmax(0,1fr)_90px_120px_150px]">
                <div className="min-w-0"><Link href={`/result/${job.id}`} className="block truncate font-medium hover:underline">{titleFor(job.sourceTitle, job.sourceText, t.untitledMaterial)}</Link><div className="mt-1 flex flex-wrap items-center gap-3"><p className="text-xs text-stone-600">{t.levelsComplete(job.completedLevels, job.levelCount)}</p><button type="button" onClick={() => reuse(job)} className="text-xs font-medium underline">{t.reuseMaterial}</button></div></div>
