...
import { validateExportPayload } from "@/lib/api/validation";
...
export async function POST(request: Request) {
  try {
    const payload = validateExportPayload(await request.json().catch(() => null));
    if (!payload) return apiError("LEVELS_INVALID");
