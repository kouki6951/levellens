...
- The application sends a restrictive Content Security Policy plus `X-Content-Type-Options`, `Referrer-Policy`, `X-Frame-Options`, and `Permissions-Policy` headers on all routes. The policy permits WebAssembly evaluation only (`wasm-unsafe-eval`) and `blob:` frames/workers for the client-side PDF preview; it does not permit JavaScript `unsafe-eval`.
